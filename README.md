...
<script>
    ...
    function generateOutgoingURL(itemId, itemName) {
        // URL mengandung parameter: action=out, id=itemId, nama=itemName (encode)
    ...
    // Generate QR Code dengan teks URL (ukuran optimal, error correction HIGH)
    ...
    function handleIncomingURL() {
        ...
        if (action === 'out' && itemId) {
            ...
            processOutgoing(itemId, 'url_scan_direct').then(() => {
                // Tampilkan feedback tambahan
                const item = items.find(i => i.id == itemId);
                if (item) {
                    showToast(`📱 Barang keluar: ${item.name} (Stok berkurang 1)`);
                }
            }).catch(console.error);
        }
    }
